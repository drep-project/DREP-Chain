...
type transport interface {
	...
	sendFindnodeHash(remote *Node, target crypto.Hash)
	sendTopicRegister(remote *Node, topics []Topic, topicIdx int, pong []byte)
	sendTopicNodes(remote *Node, queryHash crypto.Hash, nodes []*Node)
...
func (net *Network) lookup(target crypto.Hash, stopOnMatch bool) []*Node {
	var (
...
func (net *Network) reqQueryFindnode(n *Node, target crypto.Hash, reply chan []*Node) bool {
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if topicRegisterLookupTarget.target == (crypto.Hash{}) {
