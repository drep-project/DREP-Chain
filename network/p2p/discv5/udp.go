...
func ListenUDP(priv *secp256k1.PrivateKey, conn conn, nodeDBPath string, netrestrict *netutil.Netlist) (*Network, error) {
...
func listenUDP(priv *secp256k1.PrivateKey, conn conn, realaddr *net.UDPAddr) (*udp, error) {
...
func (t *udp) sendFindnodeHash(remote *Node, target crypto.Hash) {
...
func (t *udp) sendTopicNodes(remote *Node, queryHash crypto.Hash, nodes []*Node) {
...
func encodePacket(priv *secp256k1.PrivateKey, ptype byte, req interface{}) (p, hash []byte, err error) {
	b := new(bytes.Buffer)
	b.Write(headSpace)
	b.WriteByte(ptype)
...
func decodePacket(buffer []byte, pkt *ingressPacket) error {
	...
	err = binary.Unmarshal(sigdata[1:], pkt.data)
	return err
}
