...
// used buckets.
func (tab *Table) chooseBucketRefreshTarget() crypto.Hash {
	entries := 0
	if printTable {
		fmt.Println()
	}
	for i, b := range &tab.buckets {
		...
		if printTable {
			for _, e := range b.entries {
				fmt.Println(i, e.state, e.addr().String(), e.ID.String(), e.sha.String())
			}
		}
	}
...
func (tab *Table) closest(target crypto.Hash, nresults int) *nodesByDistance {
