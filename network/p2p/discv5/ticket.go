...
type ticketStore struct {
	...
	queriesSent           map[*Node]map[crypto.Hash]sentQuery
}
...
func newTicketStore() *ticketStore {
	return &ticketStore{
		...
		queriesSent:    make(map[*Node]map[crypto.Hash]sentQuery),
	}
}
...
func (s *ticketStore) addTopic(topic Topic, register bool) {
	log.WithField("topic", topic).WithField("register", register).Trace("Adding discovery topic")
...
func (s *ticketStore) removeRegisterTopic(topic Topic) {
	log.WithField("topic", topic).Trace("Removing discovery topic")
	if s.tickets[topic] == nil {
		log.WithField("topic", topic).Warn("Removing non-existent discovery topic")
		return
	}
...
func (s *ticketStore) nextRegisterLookup() (lookupInfo, time.Duration) {
	...
	for len(s.regQueue) > 0 {
		...
		if s.tickets[topic].nextLookup < mclock.Now() {
			...
			log.WithField("topic", topic).WithField("target", next.target).WithField("delay", delay).Trace("Found discovery topic to register")
			return next, delay
		}
	}
	// No registration topics found or all exhausted, sleep
	delay := 40 * time.Second
	log.WithField("delay", delay).Trace("No topic found to register")
...
func (s *ticketStore) ticketsInWindow(topic Topic) []ticketRef {
	...
	if s.tickets[topic] == nil {
		log.WithField("topic", topic).Warn("Listing non-existing discovery tickets")
		return nil
	}
	...
	log.WithField("topic", topic).WithField("from", s.lastBucketFetched).WithField("tickets", len(tickets)).Trace("Retrieved discovery registration tickets")
	return tickets
}
...
func (s *ticketStore) addTicketRef(r ticketRef) {
	...
	if tickets == nil {
		log.WithField("topic", topic).Warn("Adding ticket to non-existent topic")
		return
	}
...
func (s *ticketStore) removeTicketRef(ref ticketRef) {
	log.WithField("node", ref.t.node.ID).WithField("serial", ref.t.serial).Trace("Removing discovery ticket reference")
	...
	if tickets == nil {
		log.WithField("topic", topic).Trace("Removing tickets from unknown topic")
		return
	}
	...
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			...
			if s.canQueryTopic(n, lookup.topic) {
				...
				if hash != nil {
					s.addTopicQuery(crypto.Bytes2Hash(hash), n, lookup)
				}
			}
			//}
		}
...
func (s *ticketStore) adjustWithTicket(now mclock.AbsTime, targetHash crypto.Hash, t *ticket) {
...
func (s *ticketStore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
	log.WithField("node", ticket.node.ID).WithField("serial", ticket.serial).Trace("Adding discovery ticket")
...
func (s *ticketStore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
		log.WithField("node", node.ID).WithField("serial", nil).Trace("Retrieving node ticket")
	} else {
		log.WithField("node", node.ID).WithField("serial", s.nodes[node].serial).Trace("Retrieving node ticket")
	}
	return s.nodes[node]
}
...
func (s *ticketStore) addTopicQuery(hash crypto.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
		qq = make(map[crypto.Hash]sentQuery)
...
func (s *ticketStore) gotTopicNodes(from *Node, hash crypto.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
...
type topicRadiusBucket struct {
	...
	value      float64
	lookupSent map[crypto.Hash]mclock.AbsTime
}
...
func (r *topicRadius) getBucketIdx(addrHash crypto.Hash) int {
...
func (r *topicRadius) targetForBucket(bucket int) crypto.Hash {
...
func (r *topicRadius) isInRadius(addrHash crypto.Hash) bool {
...
func (r *topicRadius) recalcRadius() (radius uint64, radiusLookup int) {
	...
	if slopeCross != maxBucket && (minRadBucket <= maxBucket || r.needMoreLookups(maxBucket+lookupWidth, len(r.buckets)-1, maxValue)) {
		for len(r.buckets) <= maxBucket+lookupWidth {
			r.buckets = append(r.buckets, topicRadiusBucket{lookupSent: make(map[crypto.Hash]mclock.AbsTime)})
		}
...
func (r *topicRadius) adjustWithTicket(now mclock.AbsTime, targetHash crypto.Hash, t ticketRef) {
...
func (r *topicRadius) adjust(now mclock.AbsTime, targetHash, addrHash crypto.Hash, inside float64) {
