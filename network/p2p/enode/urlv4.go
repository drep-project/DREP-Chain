...
func NewV4(pubkey *secp256k1.PublicKey, ip net.IP, tcp, udp int) *Node {
	var r enr.Record
	if ip != nil {
		r.Set(enr.IP(ip))
	}
...
func parsePubkey(in string) (*secp256k1.PublicKey, error) {
	...
	b = append([]byte{0x2}, []byte(b)...)
	//b = append([]byte{0x4}, []byte(b)...)
...
func (n *Node) v4URL() string {
	...
	switch {
	case scheme == "v4" || key != secp256k1.PublicKey{}:
		//crypto.CompressPubkey()
		nodeid = fmt.Sprintf("%x", crypto.CompressPubkey(&key)[1:])
	default:
