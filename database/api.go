...
var (
    ...
    //N = 10
    //m = 0
    //RecentBlocks = make([]*bean.Block, N)
    Storages = make(map[string] *accounts.Storage)
)
...
func GetHighestBlock() *bean.Block {
    ...
    //if m < N {
    //    return RecentBlocks[m]
    //} else {
    //    return RecentBlocks[N - 1]
    //}
...
func PutBlock(block *bean.Block) error {
    ...
    //fmt.Println()
    //fmt.Println("putting block: ", block.Header.Height, "  ", block.Header.Timestamp)
    //fmt.Println("putting block: ", block.Data.TxCount)
...
func GetLastTimestamp() int64 {
    key := mycrypto.Hash256([]byte("last_timestamp"))
    if value := db.get(key); value == nil {
        return -1
    } else {
        return new(big.Int).SetBytes(value).Int64()
...
func PutLastTimestamp(timestamp int64) error {
    key := mycrypto.Hash256([]byte("last_timestamp"))
...
func PutPreviousHash(ha []byte) error {
    key := mycrypto.Hash256([]byte("previous_hash"))
    value := ha
    err := db.put(key, value, config.GetChainId())
    if err != nil {
...
func GetStorageOutsideTransaction(addr accounts.CommonAddress, chainId int64) *accounts.Storage {
    ...
    //return storage
    if s, ok := Storages[hex.EncodeToString(key)]; ok {
        return s
    } else {
        return accounts.NewStorage()
    }
}
...
func PutStorageOutsideTransaction(storage *accounts.Storage, addr accounts.CommonAddress, chainId int64) error {
    ...
    Storages[hex.EncodeToString(key)] = storage
    return nil
}
