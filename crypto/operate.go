...
var KeyInstance *common.PrivateKey
...
func GetRandomKQ() ([]byte, *common.Point, error) {
    curve := GetCurve()
...
func GetPrivateKey() (*common.PrivateKey, error) {
    var err error
    onceKey.Do(func() {
        ...
        KeyInstance = &common.PrivateKey{Prv: prv, PubKey: pubKey}
    })
    return KeyInstance, err
}

func GetPubKey() (*common.Point, error) {
...
func Sign(b []byte) (*common.Signature, error) {
    curve := GetCurve()
    prvKey, err := GetPrivateKey()
    if err != nil {
        return nil, err
    ...
    for r.Cmp(Zero) == 0 || s.Cmp(Zero) == 0 {
        ...
        N := curve.Params().N
        r = new(big.Int).SetBytes(common.ConcatHash256(Q, prvKey.PubKey, b))
        r.Mod(r, N)
...
func Verify(sig *common.Signature, pubKey *common.Point, b []byte) bool {
    curve := GetCurve()
    ...
    v := new(big.Int).SetBytes(common.ConcatHash256(Q, pubKey, b))
    v.Mod(v, N)
    if v.Cmp(r) == 0{
        return true
    } else {
...
func Encrypt(pubKey *common.Point, b []byte) ([]byte, error) {
    curve := GetCurve()
...
func Decrypt(cipher []byte) ([]byte, error) {
    ...
    p1 := &common.Point{X: cipher[:ByteLen], Y: cipher[ByteLen: 2 * ByteLen]}
