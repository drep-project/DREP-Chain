...
func Serialize(message interface{}) (*Serializable, error) {
	...
	case *Newcomer:
		serializable.Header = MsgTypeNewComer
	case *ListOfPeer:
		serializable.Header = MsgTypeUser
	case *Block:
...
func Deserialize(msg []byte) (*Serializable, interface{}, error) {
	...
	case MsgTypeNewComer:
		newcomer := &Newcomer{}
		if err := proto.Unmarshal(body, newcomer); err == nil {
			return serializable, newcomer, nil
		} else {
			return nil, nil, err
		}
	case MsgTypeUser:
