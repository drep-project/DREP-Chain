...
func (f *Filter) Logs(ctx context.Context) ([]*types.Log, error) {
	...
	blockHeader, _ := f.backend.HeaderByNumber(ctx, common.LatestBlockNumber)
	if blockHeader == nil {
		return nil, nil
	}
	head := blockHeader.Height
...
func (f *Filter) indexedLogs(ctx context.Context, end uint64) ([]*types.Log, error) {
	...
	for {
		...
		case number, ok := <-matches:
			...
			header, err := f.backend.HeaderByNumber(ctx, common.BlockNumber(number))
...
func (f *Filter) unindexedLogs(ctx context.Context, end uint64) ([]*types.Log, error) {
	...
	for ; f.begin <= int64(end); f.begin++ {
		header, err := f.backend.HeaderByNumber(ctx, common.BlockNumber(f.begin))
		if header == nil || err != nil {
			return logs, err
		}
