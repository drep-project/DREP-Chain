...
type StructLogger struct {
	...
	logs          []StructLog
	changedValues map[crypto.CommonAddress]Storage
	output        []byte
	err           error
}
...
func NewStructLogger(cfg *LogConfig) *StructLogger {
	logger := &StructLogger{
		changedValues: make(map[crypto.CommonAddress]Storage),
	}
	if cfg != nil {
		logger.cfg = *cfg
	}
...
func (l *StructLogger) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) error {
	...
	// if not present.
	if l.changedValues[contract.ContractAddr] == nil {
		l.changedValues[contract.ContractAddr] = make(Storage)
	}
	...
	if op == SSTORE && stack.len() >= 2 {
		...
		l.changedValues[contract.ContractAddr][address] = value
	}
	...
	if !l.cfg.DisableStorage {
		storage = l.changedValues[contract.ContractAddr].Copy()
	}
...
func WriteLogs(writer io.Writer, logs []*types.Log) {
	for _, log := range logs {
		fmt.Fprintf(writer, "LOG%d: %x bn=%d txi=%x\n", len(log.Topics), log.Address, log.Height, log.TxIndex)
