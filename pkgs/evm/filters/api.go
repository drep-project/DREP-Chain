...
func (api *PublicFilterAPI) NewPendingTransactionFilter() rpc.ID {
	...
	api.filtersMu.Lock()
	api.filters[pendingTxSub.ID] = &filter{typ: PendingTransactionsSubscription, deadline: time.NewTimer(deadline), hashes: make([]crypto.Hash, 0), s: pendingTxSub}
	api.filtersMu.Unlock()
...
func (api *PublicFilterAPI) NewPendingTransactions(ctx context.Context) (*rpc.Subscription, error) {
	...
	go func() {
		txHashes := make(chan []crypto.Hash, 128)
...
func (api *PublicFilterAPI) NewBlockFilter() rpc.ID {
	...
	api.filtersMu.Lock()
	api.filters[headerSub.ID] = &filter{typ: BlocksSubscription, deadline: time.NewTimer(deadline), hashes: make([]crypto.Hash, 0), s: headerSub}
	api.filtersMu.Unlock()
