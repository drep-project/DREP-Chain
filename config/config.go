...
func MakeConfig(ctx *cli.Context) ( *NodeConfig, error) {
	// Load defaults.
	nodeConfig = &NodeConfig{}
	nodeConfig.Port = defaultPort
	val := new(Blockprize)
	val.SetString(defaultBlockPrize, 10)
	nodeConfig.Blockprize = val
	//data dir setting
...
func setConsensus(ctx *cli.Context, cfg *NodeConfig) {
	if ctx.GlobalIsSet(flags.ConsensusModeFlag.Name) {
		cfg.ConsensusMode = ctx.GlobalString(flags.ConsensusModeFlag.Name)
	} else{
