...
func (context *ExecuteTransactionContext) PreCheck() error {
	...
	if nonce < context.tx.Nonce() {
		log.WithField("db nonce", nonce).WithField("tx nonce", context.tx.Nonce()).Info("state precheck too hight")
		return ErrNonceTooHigh
	} else if nonce > context.tx.Nonce() {
		log.WithField("db nonce", nonce).WithField("tx nonce", context.tx.Nonce()).Info("state precheck too low")
