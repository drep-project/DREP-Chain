...
type Database struct {
	diskdb dbinterface.KeyValueStore // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb dbinterface.KeyValueStore) *Database {
...
func NewDatabaseWithCache(diskdb dbinterface.KeyValueStore, cache int) *Database {
...
func (db *Database) DiskDB() dbinterface.KeyValueReader {
	return db.diskdb
}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > dbinterface.IdealBatchSize {
...
// Note, this method is a non-synchronized mutator. It is unsafe to call this
// concurrently with other mutators.
func (db *Database) TrieDb(node crypto.Hash, report bool) error {
	// Create a database batch to flush persistent data out. It is important that
	// outside code doesn't see an inconsistent state (referenced data removed from
	// memory cache during commit but not yet in persistent storage). This is ensured
	// by only uncaching existing data when the database write finalizes.
	...
	//	logger = log.Debug
	//}
	log.Trace("Persisted trie from memory database", "nodes", nodes-len(db.dirties)+int(db.flushnodes), "size", storage-db.dirtiesSize+db.flushsize, "time", time.Since(start)+db.flushtime,
		"gcnodes", db.gcnodes, "gcsize", db.gcsize, "gctime", db.gctime, "livenodes", len(db.dirties), "livesize", db.dirtiesSize)
...
// commit is the private locked version of TrieDb.
func (db *Database) commit(hash crypto.Hash, batch dbinterface.Batch, uncacher *cleaner) error {
